...
export default function DoctorAgendaPanel({ sessionData, submitStatus, onSubmit }) {
  const agenda = sessionData?.agenda?.length > 0 ? sessionData.agenda : MOCK_AGENDA
  const fullTranscript = sessionData?.full_q4_transcript ||
    '혈압약이랑 감기약을 같이 먹어도 되는지 궁금해요. 양파즙도 같이 먹어도 되나요?'
  const uncategorizedRemnant = sessionData?.uncategorized_remnant || ''
  ...
  const handleSubmit = () => {
    ...
    onSubmit({ answers: filled, additionalNotes })
  }
  ...
  return (
    <div className="doctor-agenda-panel">
      ...
      <section className="dap-full-quote dap-full-quote-v4">
        ...
        <p className="dap-quote-content-v4">"{fullTranscript}"</p>
      </section>
      ...
      {/* ─── 진료 메모 ─── */}
      <section className="dap-notes-block">
        <label className="dap-label">진료 메모 (선택 · 환자 안내문에 표시 안 됨)</label>
        <textarea
          className="dap-textarea dap-notes"
          placeholder="다음 진료 일정, X-ray 의뢰, 처방 메모 등"
